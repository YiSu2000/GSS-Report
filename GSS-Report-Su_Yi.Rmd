...
output: pdf_document
---
### Do people above age of 30 with higher income more likely to be married or has married, comparing to those with lower income in Canada?

```{r message=FALSE, warning=FALSE, include=FALSE}
###The code in this code chunk are created by Rohan Alexander and Sam Caetano with Licence: MIT. 
###All file used in this code chunk and the original code by Rohan Alexander and Sam Caetano are included in the github repo file "Prerequisite".
#### Workspace set-up ####
library(janitor)
library(tidyverse)

# Load the data dictionary and the raw data and correct the variable names
raw_data <- read_csv("/Users/jordans2000/Desktop/STA304/A3-GSS report/GSS-Report/Prerequisite/AAqwiAeV.csv")
dict <- read_lines("/Users/jordans2000/Desktop/STA304/A3-GSS report/GSS-Report/Prerequisite/gss_dict.txt", skip = 18) # skip is because of preamble content
# Now we need the labels because these are the actual responses that we need
labels_raw <- read_file("/Users/jordans2000/Desktop/STA304/A3-GSS report/GSS-Report/Prerequisite/gss_labels.txt")
...
MarIA <- MarIA%>%select(age, marital_status, income_respondent)
MarIA <- MarIA%>%replace_with_na(replace = list(marital_status = "NA"))
MarIA <- MarIA%>%na.omit()
## To get only the variables we are interested in
MarIA <- MarIA%>%transform(ever_married = ifelse(marital_status =="Single, never married", yes = "No", no = "Yes"))
...
## having a conjugal relationship.
MarIA$income_respondent_f <- factor(MarIA$income_respondent)
summary(MarIA)
MarIA%>%count(income_respondent)
```
## Model
```{r}
library(rstan)
...
## the residual variance, because in the logistic model, this is the constant π^2/3.
fit1 <- brm(ever_married~age+income_respondent_f, 
            data = MarIA, 
            family = bernoulli(),
            prior = set_prior("normal(0,1)", class="b"),
            seed = 416)
## using dummy variable coding to income_respondent_f
summary(fit1)
